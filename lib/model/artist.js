...
class ArtistModel extends BaseModel {
    ...
    getArtist(artistUrl) {
        ...
        bandcamp.getCache().cacheOrPromise(this.getCacheKeyForFetch('artist', { artistUrl }), () => {
            return bcfetch.getArtistOrLabelInfo(artistUrl);
        }).then( (info) => {
    ...
    getFetchPromise(options) {
        return bandcamp.getCache().cacheOrPromise(this.getCacheKeyForFetch('artists', { labelUrl: options.labelUrl }), () => {
            return bcfetch.getLabelArtists(options.labelUrl);
        });
    }
